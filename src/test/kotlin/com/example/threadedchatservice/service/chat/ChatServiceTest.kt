...
class ChatServiceTest {
    ...
    fun setUp() {
        ...
        userRepository = mock()
        claudeClient = mock()
        chatService = ChatService(chatRepository, threadRepository, userRepository, claudeClient)
    }
    ...
    inner class PrepareChatTest {
        ...
        fun `given first question, when prepareChat, then creates new thread`() {
            ...
            // when
            val result = chatService.prepareChat(1L, request)
        ...
        fun `given non-existent user, when prepareChat, then throws exception`() {
            ...
            val exception = assertThrows(IllegalArgumentException::class.java) {
                chatService.prepareChat(999L, request)
            }
    ...
    inner class CreateChatTest {
        ...
        fun `given non-streaming request, when createChat, then returns Complete`() {
            ...
            whenever(userRepository.findById(1L)).thenReturn(Optional.of(user))
            whenever(threadRepository.findLatestByUserId(1L)).thenReturn(null)
            whenever(threadRepository.save(any<ThreadEntity>())).thenReturn(thread)
            whenever(chatRepository.findByThreadIdOrderByCreatedAtAsc(1L)).thenReturn(emptyList())
            whenever(claudeClient.call(any<List<Map<String, String>>>(), org.mockito.kotlin.isNull())).thenReturn("답변")
            whenever(chatRepository.save(any<ChatEntity>())).thenReturn(savedChat)
